...
from .models import ObservationAttached, ObservationDetached, Observation, Event
...
class ComplexPresenter:
    ...
    def _get_attached_stream_ids(self):
        ...
        attach_detach_events = Event.objects.instance_of(
            ObservationAttached,
            ObservationDetached
        ).filter(
            event_stream_id=self.observation_event_stream_id
        ).order_by('published')
