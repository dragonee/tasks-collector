...
from .forms import OnlyTextSingleHabitTrackedForm, SingleHabitTrackedForm
from .models import Habit, HabitTracked, Profile, Thread
from .serializers import HabitKeywordSerializer, HabitSerializer
...
class HabitViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .annotate(
                today_tracked=Count(
                    "habittracked", filter=Q(habittracked__published__date=day)
                ),
            )
        )
...
def _habit_calendar(habit, start, end):
    events = (
        HabitTracked.objects.filter(
            habit=habit,
            published__range=(start, end),
        )
        .order_by("published")
        .values("published", "occured")
    )
...
def get_habit_keywords_for_user(user, all=False):
    ...
    def profile_has_keywords(user):
        return (
            Profile.objects.annotate(keyword_count=Count("habit_keywords"))
            .filter(user=user, keyword_count__gt=0)
            .exists()
        )
...
def get_habit_keywords_for_user_and_date(user, date, all=False):
    ...
    tracked_habit_ids = (
        HabitTracked.objects.filter(published__date=date)
...
def my_habit_keywords(request):
    ...
    habit_keywords = get_habit_keywords_for_user_and_date(
        request.user, target_date, all=request.query_params.get("all") == "true"
    )

    serializer = HabitKeywordSerializer(
        habit_keywords, many=True, context={"request": request}
    )
