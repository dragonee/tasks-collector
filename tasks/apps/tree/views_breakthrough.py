...
def breakthrough(request, year):
    ...
    ProjectedOutcomeFormSet = inlineformset_factory(
        Breakthrough, ProjectedOutcome, form=ProjectedOutcomeForm, extra=1
    )
    projected_outcome_queryset = ProjectedOutcome.objects.filter(
        breakthrough=breakthrough
    ).order_by("resolved_by")
...
def projected_outcome_events_history(request, event_stream_id):
    ...
    return render(
        request,
        "tree/projected_outcome_events_history.html",
        {
            ...
            "latest_closed_event": (
                presentation.closed_events[-1] if presentation.closed_events else None
            ),
...
def projected_outcome_close(request, projected_outcome_id):
    ...
    projected_outcome_closed = ProjectedOutcomeClosed.from_projected_outcome(
        projected_outcome
    )
    ...
    response["HX-Redirect"] = reverse(
        "breakthrough", args=[projected_outcome.breakthrough.slug]
    )
