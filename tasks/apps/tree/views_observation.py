...
from .observation_operations import (
    migrate_observation_updates_to_journal as _migrate_observation_updates_to_journal,
)
...
class ObservationFilter(filters.FilterSet):
    ...
    class Meta:
        model = Observation
        fields = {"pub_date": ("gte", "lte"), "event_stream_id": ("exact",)}
...
class EventFilter(filters.FilterSet):
    ...
    def filter_open(self, queryset, name, value):
        ...
        return queryset.annotate(
            is_open=Exists(
                Observation.objects.filter(event_stream_id=OuterRef("event_stream_id"))
            )
...
def observation_edit(request, observation_id=None):
    ...
    ObservationUpdatedFormSet = inlineformset_factory(
        Observation, ObservationUpdated, fields=("comment",), extra=3
    )
...
def observation_attach(request, observation_id):
    ...
    if other_observation_id:
        ...
        try:
            other_observation = Observation.objects.get(
                event_stream_id=other_event_stream_id
            )
        except Observation.DoesNotExist:
    ...
    if complex_presenter.is_attached(other_event_stream_id):
        ...
        if latest_attach_event:
            serializer = ObservationAttachedSerializer(
                latest_attach_event, context={"request": request}
            )
    ...
    serializer = ObservationAttachedSerializer(
        attach_event, context={"request": request}
    )
    return RestResponse(serializer.data, status=status.HTTP_201_CREATED)
...
def observation_detach(request, observation_id):
    ...
    if not complex_presenter.is_attached(other_event_stream_id):
        ...
        latest_detach_event = (
            ObservationDetached.objects.filter(
                event_stream_id=complex_observation.event_stream_id,
                other_event_stream_id=other_event_stream_id,
            )
            .order_by("-published")
        ...
        if latest_detach_event:
            serializer = ObservationDetachedSerializer(
                latest_detach_event, context={"request": request}
            )
            return RestResponse(serializer.data, status=status.HTTP_201_CREATED)
    ...
    serializer = ObservationDetachedSerializer(
        detach_event, context={"request": request}
    )
    return RestResponse(serializer.data, status=status.HTTP_201_CREATED)
...
def observation_search(request):
    ...
    observation_id = request.GET.get(
        "observation"
    )  # Optional parameter to filter out attached observations
    ...
    if pk_query.isdigit():
        ...
        observations = (
            Observation.objects.with_last_event_published()
            .extra(where=["CAST(id AS TEXT) LIKE %s"], params=[pk_query + "%"])
            .order_by("id")
        )
        ...
        serializer = ObservationSerializer(
            observations, many=True, context={"request": request}
        )
        return RestResponse(
            {
